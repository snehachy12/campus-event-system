...
export default function WorkshopsPage() {
  ...
  useEffect(() => {
    const fetchWorkshops = async () => {
      try {
        const res = await fetch('/api/partcipant/events?type=workshop') 
    ...
    <div className="min-h-screen bg-black flex">
      ...
      <main className="flex-1 overflow-auto">
        <header className="sticky top-0 z-10 border-b border-zinc-800 bg-zinc-900/30 backdrop-blur px-8 py-6 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-white mb-1">Workshops & Bootcamps</h1>
            <p className="text-zinc-400 text-sm">Learn new skills from industry experts and seniors.</p>
          </div>
