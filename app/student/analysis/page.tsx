...
export default function StudentAnalysisPage() {
  ...
  useEffect(() => {
    ...
    if (userStr) {
      ...
      if (storedPersona === "organizer") {
        fetchDashboardData(user._id || user.id, storedPersona)
      }
    }
    // eslint-disable-next-line
  }, [])

  const fetchDashboardData = async (userId: string, personaType: string) => {
    try {
      const res = await fetch(`/api/organizer/dashboard?userId=${userId}&persona=${personaType}`)
  ...
  const handlePersonaChange = (val: string) => {
    ...
    if (userStr) {
      const user = JSON.parse(userStr)
      if (val === "organizer") {
        fetchDashboardData(user._id || user.id, val)
      }
    }
  }
    ...
    return (
      <div className="min-h-screen bg-black flex">
        ...
        <main className="flex-1 overflow-auto flex flex-col items-center justify-center">
          <div className="p-8">
            <PersonaSelector value={persona} onChange={handlePersonaChange} allowedPersonas={["student", "organizer"]} />
            <h2 className="text-2xl text-white mt-8">Switch to Organizer persona to view Analysis</h2>
          </div>
        </main>
      </div>
    )
        ...
        <header className="sticky top-0 z-10 border-b border-zinc-800 bg-zinc-900/30 backdrop-blur px-8 py-6 flex justify-between items-center">
          <div>
            ...
            <p className="text-zinc-400 text-sm">
              Welcome back, <span className="text-[#e78a53] font-medium">{studentName}</span>
            </p>
          </div>
                ...
                <div className="h-75 w-full">
                  {loading || !data ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={data.charts.revenueTrend} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                        <defs>
                          <linearGradient id="colorRevenue" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="5%" stopColor="#e78a53" stopOpacity={0.8}/>
                        ...
                        <YAxis tick={{ fill: '#a1a1aa', fontSize: 12 }} axisLine={false} tickFormatter={(value) => `₹${value}`} />
                        <CartesianGrid strokeDasharray="3 3" stroke="#27272a" vertical={false} />
                        <Tooltip contentStyle={{ backgroundColor: '#18181b', borderColor: '#27272a', borderRadius: '8px' }} itemStyle={{ color: '#fff' }} />
                        <Area type="monotone" dataKey="revenue" stroke="#e78a53" strokeWidth={2} fillOpacity={1} fill="url(#colorRevenue)" />
                      </AreaChart>
                    </ResponsiveContainer>
                  )}
                </div>
                  ...
                  {loading || !data ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={data.charts.eventPerformance} layout="vertical" margin={{ left: 0 }}>
                        ...
                        <YAxis dataKey="name" type="category" width={100} tick={{ fill: '#a1a1aa', fontSize: 12 }} tickLine={false} axisLine={false} />
