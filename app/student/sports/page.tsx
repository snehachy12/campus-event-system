...
export default function SportsPage() {
  ...
  useEffect(() => {
    const fetchSports = async () => {
      try {
        const res = await fetch('/api/partcipant/events?type=sports') 
        const data = await res.json()
        if (data.success) {
          setEvents(data.events)
        }
      } catch (error) {
        console.error("Failed to fetch sports events")
      } finally {
        setLoading(false)
      }
    }
    ...
    <div className="min-h-screen bg-black flex">
      ...
      <main className="flex-1 overflow-auto">
        <header className="sticky top-0 z-10 border-b border-zinc-800 bg-zinc-900/30 backdrop-blur px-8 py-6 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-white mb-1">Sports Events</h1>
            <p className="text-zinc-400 text-sm">Compete and showcase your athletic prowess.</p>
          </div>
        ...
        <div className="p-8">
          {loading ? (
            ...
            <div className="text-zinc-500 text-center py-20 bg-zinc-900/30 rounded-xl border border-zinc-800">
               No sports events scheduled right now.
            </div>
          ) : (
                ...
                <Card key={event._id} className="bg-zinc-900/50 border-zinc-800 hover:border-[#e78a53]/50 transition-colors flex flex-col">
                  <CardHeader>
                    <div className="flex justify-between items-start mb-2">
                      <Badge className="bg-yellow-500/10 text-yellow-500 border-yellow-500/20">Sports</Badge>
